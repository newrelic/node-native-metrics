...
function install(target, cb) {
  ...
  if (opts['no-build'] || process.env.NR_NATIVE_METRICS_NO_BUILD) {
    return doDownload()
  }
  ...
  function doDownload() {
    if (
      opts['no-download'] || process.env.NR_NATIVE_METRICS_NO_DOWNLOAD
      && !opts['no-build'] && !process.env.NR_NATIVE_METRICS_NO_BUILD
    ) {
